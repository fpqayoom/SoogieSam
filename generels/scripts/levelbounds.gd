@tool
@icon("res://generels/icons/level_bounds.svg")
class_name LevelBounds extends Node2D

@export_range(400, 2070, 32, "Suffix:px") var width :int = 400 : set = _on_width_changed
@export_range(270, 2070, 32, "Suffix:px") var height : int = 270 : set = _on_height_changed
...
func _draw() -> void:
	if Engine.is_editor_hint():
		...
		draw_rect(r, Color("#00FFFF80"), false, 4)
		draw_rect(r, Color.BLUE, false, 2 )
	pass
...
func _on_height_changed(new_hieght: int)-> void:
	height = new_hieght
	queue_redraw()
	pass
